...
Section('supcon', 'supcon').params(
    temperature=Param(float, 'SupCon temperature', default=0.07),
)
...
class ImageNetTrainer:
    ...
    def __init__(self, gpu, ngpus_per_node, world_size, dist_url, distributed, batch_size, label_smoothing, loss, train_probes_only, epochs, train_dataset, val_dataset):
        ...
        if loss == "simclr":
            ...
            self.ssl_loss = SupConLoss(temperature=0.1).to(self.gpu)
        elif loss == "vicreg":
    ...
    def train_loop(self, epoch, log_level, base_lr, end_lr_ratio):
        ...
        for ix, loaders in enumerate(iterator, start=epoch * len(self.train_loader)):
            ...
            labels_big = loaders[1] # Expected shape (batch_size, 40)
            ...
            if self.do_ssl_training:
                ...
                with autocast():
                    ...
                    if self.teacher_student:
                        ...
                        elif "supcon" in self.loss_name:
                            f1, f2 = ch.split(embedding_big, [batch_size, batch_size], dim=0)
                            features = ch.cat([f1.unsqueeze(1), f2.unsqueeze(1)], dim=1)
                            loss_train = [self.ssl_loss(features, labels_big[:,i]) for i in range(40)]
                            loss_train = ch.stack(loss_train).mean()
                        else:
                            embedding_big = embedding_big.view(2, batch_size, -1)
            ...
            with autocast():
                ...
                for l in range(len(list_outputs)):
                    ...
                    current_loss = self.loss(list_outputs[l], labels_big[:,20])
                    ...
                    for k in ['top_1_layer'+str(l), 'top_5_layer'+str(l)]:
                        self.train_meters[k](list_outputs[l].detach(), labels_big[:, 20])
...
def make_config(quiet=False):
    ...
    # parser.add_argument("folder", type=str, default="ssl-logs/combined-supcon")
