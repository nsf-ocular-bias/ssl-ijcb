...
class SupConLoss(nn.Module):
    ...
    def __init__(self, temperature=0.07, contrast_mode='all',
                 base_temperature=0.07, topk=False):
    ...
    def forward(self, features, labels=None, mask=None):
        ...
        if labels is not None and mask is not None:
            ...
            mask = ch.eye(batch_size, dtype=ch.float32).to(device)
        elif labels is not None:
            labels = labels.contiguous().view(-1, 1)
            if labels.shape[0] != batch_size:
                raise ValueError('Num of labels does not match num of features')
            mask = ch.eq(labels, labels.T).float().to(device)
        else:
            mask = mask.float().to(device)

        contrast_count = features.shape[1]
        contrast_feature = ch.cat(ch.unbind(features, dim=1), dim=0)
        ...
        # compute logits
        anchor_dot_contrast = ch.div(
            ch.matmul(anchor_feature, contrast_feature.T),
            self.temperature)
        # for numerical stability
        logits_max, _ = ch.max(anchor_dot_contrast, dim=1, keepdim=True)
        logits = anchor_dot_contrast - logits_max.detach()

        # tile mask
        mask = mask.repeat(anchor_count, contrast_count)
        # mask-out self-contrast cases
        logits_mask =ch.scatter(
            ch.ones_like(mask),
            1,
            ch.arange(batch_size * anchor_count).view(-1, 1).to(device),
            0
        )
        mask = mask * logits_mask

        # compute log_prob
        exp_logits = ch.exp(logits) * logits_mask
        log_prob = logits - ch.log(exp_logits.sum(1, keepdim=True))
        ...
        mask_pos_pairs = ch.where(mask_pos_pairs < 1e-6, 1, mask_pos_pairs)
        mean_log_prob_pos = (mask * log_prob).sum(1) / mask_pos_pairs

        # loss
        loss = - (self.temperature / self.base_temperature) * mean_log_prob_pos
        if self.topk:
            # Find kth largest loss
            kth_largest = ch.topk(loss, 64, largest=False)[0][-1]
            loss = ch.abs(loss - kth_largest) + kth_largest
